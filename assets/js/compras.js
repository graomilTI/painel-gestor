...
function renderMinhas(){
  ...
  body.innerHTML=data.map(r=>`<tr><td>${brDate(r.data_solicitacao)}</td><td>${esc(r.tipo_solicitacao)}</td><td><div class="cmp-items-cell">${(r.compras_itens||[]).map(i=>`<span>${esc(i.quantidade||i.unidade||1)} un · ${esc(i.material)}${i.tamanho?` <small>${esc(i.tamanho)}</small>`:''}</span>`).join('')}</div></td><td>${pill(r.status)}</td><td>${esc(r.motivo_recusa||'—')}</td></tr>`).join('');
}
...
.cmp-workspace{display:grid;grid-template-columns:minmax(360px,430px) minmax(0,1fr);gap:16px;align-items:start}.cmp-request-card,.cmp-history-card{margin-top:16px;min-width:0}.cmp-request-card{position:sticky;top:16px}.cmp-request-card .section-head{align-items:flex-start;flex-direction:column}.cmp-request-card .cmp-tabs{order:-1}.cmp-request-card .cmp-grid{grid-template-columns:1fr}.cmp-request-card .cmp-add-box{grid-template-columns:82px minmax(0,1fr)}.cmp-request-card .cmp-add-box .cmp-field:nth-child(3),.cmp-request-card .cmp-add-box .cmp-field:nth-child(4),.cmp-request-card .cmp-add-action{grid-column:1/-1}.cmp-request-card .cmp-add-action label{display:none}.cmp-request-card .cmp-add-action .btn,.cmp-request-card #cmpSolicitar{width:100%}.cmp-request-card .cmp-table{min-width:560px}.cmp-request-card .cmp-table-wrap{max-height:260px}.cmp-request-card #panel-uniformes .cmp-actions{display:grid}.cmp-request-card #panel-uniformes .cmp-field{min-width:0!important}.cmp-history-head{align-items:flex-end}.cmp-history-filters{display:flex;gap:8px;flex-wrap:wrap}.cmp-history-filter{width:auto!important;margin:0!important;border-radius:999px!important;padding-inline:16px!important}.cmp-history-filter.active{background:#166534!important;border-color:#22c55e!important;color:#fff!important}.cmp-history-card .cmp-table-wrap{border-radius:18px}.cmp-history-card .cmp-table{min-width:760px}.cmp-history-card .cmp-table th{padding-top:16px;padding-bottom:16px}.cmp-history-card .cmp-table td{padding-top:18px;padding-bottom:18px}.cmp-items-cell{display:grid;gap:5px}.cmp-items-cell span{display:block}.cmp-items-cell small{color:var(--muted);margin-left:4px}.cmp-request-card .form-actions{display:grid;gap:10px}.cmp-feedback{line-height:1.45}
@media(max-width:1180px){.cmp-workspace{grid-template-columns:minmax(330px,380px) minmax(0,1fr)}.cmp-request-card .cmp-add-box{grid-template-columns:1fr}}
...
@media(max-width:760px){.cmp-grid,.cmp-add-box,.cmp-request-card .cmp-add-box{grid-template-columns:1fr}.cmp-request-card .cmp-add-box .cmp-field{grid-column:1!important}.cmp-table{min-width:680px}.cmp-history-filters{width:100%}.cmp-history-filter{flex:1;padding-inline:10px!important}}
</style>`}
...
initProtectedPage('Compras', async (content, userContext)=>{
  ...
  <div class="cmp-workspace"><section class="card cmp-request-card"><div class="section-head"><div><h3>Nova solicitação</h3><p class="muted">Solicitante: <b>${esc(solicitanteNome(userContext))}</b></p></div><div class="cmp-tabs"><button class="btn btn-secondary cmp-tab active" data-mode="itens" type="button">MATERIAL</button><button class="btn btn-secondary cmp-tab" data-mode="uniformes" type="button">UNIFORME</button></div></div>
  ...
  <section class="card cmp-history-card"><div class="section-head cmp-history-head"><div><h3>Pendentes e histórico</h3><p class="muted">Acompanhe o andamento de tudo que já foi solicitado.</p></div><button class="btn btn-secondary" id="cmpRefresh" type="button">Atualizar</button></div><div class="cmp-history-filters"><button class="btn btn-secondary cmp-history-filter active" data-history-filter="pendentes" type="button">Pendentes</button><button class="btn btn-secondary cmp-history-filter" data-history-filter="concluidos" type="button">Concluídos</button><button class="btn btn-secondary cmp-history-filter" data-history-filter="cancelados" type="button">Cancelados</button></div><div class="cmp-table-wrap mt-16"><table class="cmp-table"><thead><tr><th>Data</th><th>Tipo</th><th>Itens</th><th>Status</th><th>Motivo</th></tr></thead><tbody id="cmpMinhasBody"></tbody></table></div></section></div>`;
