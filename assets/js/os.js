...
initProtectedPage('OS', async (content) => {
  ...
  function rowHtml(row) {
    ...
    const acaoCol = isNegativo
      ...
      : zero
      ? `<div class="os-actions"><button class="os-btn danger ${status === 'FINALIZAR' ? 'active' : ''}" data-status="FINALIZAR" title="Finalizar">${ICO_FINALIZAR}</button><button class="os-btn os-btn-kg ${row.observacao_logistica?.startsWith('KG solicitado') ? 'active' : ''}" data-kg-id="${escapeHtml(String(row.id))}" data-kg-num="${escapeHtml(row.numero_os)}" title="Solicitar aumento de saldo">${ICO_SOMAR_KG}</button></div><div style="margin-top:8px"><span class="os-chip ${statusClass(row)}">${escapeHtml(status)}</span></div>`
